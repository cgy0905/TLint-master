...
public class HtmlUtil {
    ...
    public static String getHtmlString(Context context) {
        ...
        String assetHtml = FileUtil.stringFromAssetsFile(context, "hupu_thread.html");
        ...
        if (!TextUtils.isEmpty(assetHtml)) {
            htmlString = assetHtml.replace("{hupu.js}",
                    ...
                    .replace("{jockey.js}", String.format("file://%s", cachePath + File.separator + "jockey.js"))
                    .replace("{zepto.js}", String.format("file://%s", cachePath + File.separator + "zepto.js"));
        }
        return htmlString;
    }
