...
import com.cgy.hupu.bean.ThreadReplyResult;
import com.cgy.hupu.components.notifier.OfflineNotifier;
...
public class OfflineService extends Service {
    ...
    private long offlineRepliesLength = 0;// 离线的回复总流量大小
    ...
    private void getThreadList(final Forum forum, final String lastTid, final String lastStm) {
        ...
        Subscription mSubscription = mForumApi.getThreadsList(fid, lastTid, lastStm,
                ...
                .subscribe(threadListData -> getThreadList(forum, lastTidMap.get(fid), lastStmMap.get(fid)), throwable -> {
    ...
    private void prepareReplies() {
        ...
        if (threadList.isEmpty()) {
            ...
            for (final Thread thread : threadList) {
                Subscription mSubscription = Observable.zip(mForumApi.getThreadInfo(thread.getTid(), thread.getFid(), 1, ""),
                        ...
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe(aBoolean -> offlineReplyComplete(thread), throwable -> {
                            throwable.printStackTrace();
